...
class MapsMidiRandomDataset(torch.utils.data.Dataset):
    def __init__(self, data_dir, sample_rate, midi_rate, sample_length, epoch_size, normalize=True, mode: str = "train", year: str = None):
        if data_dir[-1] != "/":
            data_dir = data_dir + "/"
        ...
        self.weights = [midi.shape[0]//self.sample_length for midi in self.midi]
    ...
    def __getitem__(self, idx):
        selected = random.choices(range(len(self.midi)), weights=self.weights, k=1)[0]

        start = random.randint(0, self.midi[selected].shape[0] - int(self.midi_rate * self.sample_length))
        start_audio = start * (self.sample_rate//self.midi_rate)

        audio = self.audio[selected][start_audio:start_audio + int(self.sample_rate * self.sample_length)].float() / 2**15
        audio = (audio - self.audio_mean) / self.audio_std

        midi = self.midi[selected][start:start + int(self.midi_rate * self.sample_length)].float() / 127

        year = self.year[selected]
